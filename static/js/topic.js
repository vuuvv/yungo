...
;(function($){
    $.yungo.topic = {
        ...
        init: function(options){
            options && $.extend($.yungo.topic.settings, options);
            var s = $.yungo.topic.settings;
            $(s.topic_unit).live({
                mouseover: function(){
            ...
            $.yungo.topic.forward();
            $.yungo.topic.comment();
            $.yungo.topic.comment_sub();
            $.yungo.topic.comment_reply();
            $.yungo.topic.comment_del();
            $.yungo.topic.comment_me_reply();
            $.yungo.topic.comment_me_reply_sub();
        },
        //点击缩略查看详细
        piczoom: function(){
            var s = $.yungo.topic.settings;
            $(s.topic_unit).find('.J_pic').live('click', function(){
                ...
                if(self.siblings('.J_src').html()){
                    ...
                    $.getJSON(YUNGO.root + '/?m=space&a=feed_src', {src_id:src_id, src_type:src_type}, function(result){
                        if(result.status == 1){
                            self.hide();
                            self.siblings('.J_src').html(result.data).show();
                            $.yungo.ui.decode_img(self.siblings('.J_src'));
                        }else{
        ...
        delete: function(){
            ...
            $(s.topic_unit).find('.J_del').live('click', function(){
                ...
                $.getJSON(YUNGO.root + '/?m=topic&a=delete', {tid:tid}, function(result){
                    if(result.status == 1){
        ...
        forward: function(){
            ...
            $(s.topic_unit).find('.J_fw').live('click', function(){
                ...
                $.getJSON(YUNGO.root + '/?m=topic&a=forward', {tid:tid}, function(result){
                    if(result.status == 1){
                        $.dialog.get('topic_forward').content(result.data);
                        $.yungo.topic.forward_form($('#J_forward_form'));
                    }else{
                        $.yungo.tip({content:result.msg, icon:'error'});
                    }
                });
        ...
        forward_form: function(form){
            form.ajaxForm({
                beforeSubmit: function(){
                    ...
                    if(content == ''){
                        $.yungo.tip({content:lang.please_input+lang.forward_content, icon:'error'});
                        return !1;
                    }
                },
        ...
        comment_list: function(unit){
            ...
            $.getJSON(YUNGO.root + '/?m=topic&a=comment', {tid:tid}, function(result){
                if(result.status == 1){
        ...
        comment_sub: function(){
            ...
            $(s.cmt_pub).find('.J_cmt_sub').live('click', function(){
                ...
                $.ajax({
                    url: YUNGO.root + '/?m=topic&a=comment',
                    type: 'POST',
                    data: {
                        tid: tid,
                        content: content
        ...
        comment_del: function(){
            ...
            $(s.cmt_unit).find('.J_cmt_del').live('click', function(){
                ...
                $.getJSON(YUNGO.root + '/?m=topic&a=comment_del', {cid:cid}, function(result){
                    if(result.status == 1){
                        cmt_i.remove();
                    }else{
                        $.yungo.tip({content:result.msg, icon:'error'});
